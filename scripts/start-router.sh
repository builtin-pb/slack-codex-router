...
write_launchd_plist() {
  ...
  stdout_log=$3
  stderr_log=$4
  command_string="cd $(shell_quote "$ROOT_DIR") && export SCR_ROOT_DIR=$(shell_quote "$ROOT_DIR") && exec $(shell_quote "$uv_bin") run slack-codex-router run"
...
ExecStart=$uv_bin run slack-codex-router run
Restart=always
RestartSec=5
...
case "$OS_NAME" in
  ...
  Linux)
    if command -v systemctl >/dev/null 2>&1 && systemctl --user show-environment >/dev/null 2>&1; then
      ...
      cd "$ROOT_DIR"
      exec "$UV_BIN" run slack-codex-router run
    fi
    ;;
  *)
